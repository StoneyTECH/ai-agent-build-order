...
The nine gates are mapped. Design and acceptance criteria in
[docs/atlas-crosswalk-plan.md](docs/atlas-crosswalk-plan.md); the block and the
edges live in `crosswalk.v1.json`, regenerated by `scripts/map-gates.mjs`.

| | |
|---|---|
| mitigation-backed edges | 15 |
| asserted edges | 9 |
| techniques addressed | 61 of 170 |
| open | 109 |
| gates with no ATLAS edge | gate:8, gate:9 |

**Most of ATLAS is out of scope for a build-time audit, and the table says so.**
Training-time poisoning, model theft, and physical-world evasion are not things
nine build gates have an opinion about. Reporting them open is the correct
result, not a gap to paper over.

**Gates 8 and 9 carry no ATLAS edges at all.** Turning failures into fixtures,
and building a way home, are engineering disciplines ATLAS has no vocabulary
for. Inventing edges so no gate looks empty would be exactly the laundering the
two-class design exists to prevent.
